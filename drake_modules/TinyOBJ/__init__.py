...
class TinyOBJ(Module):

  EXPECTED = {
    "1.0.6": {
      "headers": ["tiny_obj_loader.h"],
      "libraries": {
        "linux": ["libtinyobjloader.a"]
      },
      "others": []
    }
  }
  ...
  def __init__(self,
               ...
               base_url = None,
               platform = "linux",
               dest = drake.Path("TinyObj"),
               expected_headers = None):
    super().__init__("TinyOBJ")
    self.__base_url = base_url or "https://github.com/syoyo/tinyobjloader/archive"
    self.__version = version
    self.__platform = "linux"
    self.__tar = drake.Node(dest  / "v{version}.tar.gz".format(version = self.__version))
    self.__path = dest / "tinyobjloader-{version}".format(version = self.__version)
    ...
    drake.ShellCommand(
      ...
      command = ['make', 'tinyobjloader'],
      cwd = self.__path)

  @property
  def headers(self):
    return drake.nodes(*[self.__include_path / f for f in TinyOBJ.EXPECTED[self.__version]["headers"]])

  @property
  def libraries(self):
    return drake.nodes(*[self.__library_path / f for f in TinyOBJ.EXPECTED[self.__version]["libraries"][self.__platform]])
